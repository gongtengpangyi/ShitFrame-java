...
public class BaseDaoImpl<T extends Serializable, E extends Serializable> extends HibernateDaoSupport
	...
	public List<T> pageableFindByHQL(Pager pager, String hql, Map<String, Object> queryParams) {
		...
		if (pager != null) {
			...
			if (hql.indexOf("group by") != -1) {
				// queryCount= this.getSession().createQuery("select count(o)
				// from (" + hql + ")");
				...
				queryCount = this.getSession()
						.createQuery("select count(o) from " + this.t.getName() + " o where 1=1 " + where);
	...
	private void setQueryNamedParamters(Map<String, Object> queryParams, Query queryList) {
		if (queryParams != null && !queryParams.isEmpty()) {
			Set<Entry<String, Object>> set = queryParams.entrySet();
			for (Entry<String, Object> entry : set) {
	 ...
	 * 
	 * @see
	 * com.ali.b2b.crm.workflow.dao.BaseDao#pageableFindByHQL(com.ali.b2b.crm.
	 * workflow.model.Pager, java.lang.String)
	 */
	...
	public List<T> pageableFindByNativeSql(Pager pager, String sql, Map<String, Object> queryParams) {
	...
	public List<T> findByExample(T model, Pager pager) {
		// Example example =
		// Example.create(model).excludeZeroes().ignoreCase().enableLike(MatchMode.ANYWHERE);
		// return
		// this.getHibernateTemplate().findByExample(t.getName(),example,pager.getStart(),pager.getPageSize());
